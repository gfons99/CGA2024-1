...
// Shader con múltiples luces y múltiples texturas
Shader shaderMulLightingMT;
// Shader con múltiples luces y múltiples texturas
Shader shaderMulLighting_T03;
...
GLuint textureWater2ID, textureSpongeID;
...
glm::mat4 modelMatrix_boxEsponja = glm::mat4(1.0f);
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	shaderMulLighting.initialize(
		"../Shaders/iluminacion_textura_animation.vs", 
		"../Shaders/multipleLights.fs");
	shaderMulLightingMT.initialize(
		"../Shaders/iluminacion_textura_animation.vs",
		"../Shaders/multipleLights_ejercicio_mt.fs");
	shaderMulLighting_T03.initialize(
		"../Shaders/iluminacion_textura_animation.vs",
		"../Shaders/multipleLights_tarea03.fs");
	...
	boxEsponja.init();
	boxEsponja.setShader(&shaderMulLighting_T03);
	...
	// Definiendo la textura a utilizar
	Texture textureWater2("../Textures/water2.jpg");
	// Carga el mapa de bits (FIBITMAP es el tipo de dato de la libreria)
	textureWater2.loadImage();
	// Creando la textura con id 1
	glGenTextures(1, &textureWater2ID);
	// Enlazar esa textura a una tipo de textura de 2D.
	glBindTexture(GL_TEXTURE_2D, textureWater2ID);
	// set the texture wrapping parameters
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT); // set texture wrapping to GL_REPEAT (default wrapping method)
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
	// set texture filtering parameters
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	// Verifica si se pudo abrir la textura
	if (textureWater2.getData()) {
		// Transferis los datos de la imagen a memoria
		// Tipo de textura, Mipmaps, Formato interno de openGL, ancho, alto, Mipmaps,
		// Formato interno de la libreria de la imagen, el tipo de dato y al apuntador
		// a los datos
		glTexImage2D(GL_TEXTURE_2D, 0, textureWater2.getChannels() == 3 ? GL_RGB : GL_RGBA, textureWater2.getWidth(), textureWater2.getHeight(), 0,
		textureWater2.getChannels() == 3 ? GL_RGB : GL_RGBA, GL_UNSIGNED_BYTE, textureWater2.getData());
		// Generan los niveles del mipmap (OpenGL es el ecargado de realizarlos)
		glGenerateMipmap(GL_TEXTURE_2D);
	} else
		std::cout << "Failed to load texture" << std::endl;
	// Libera la memoria de la textura
	textureWater2.freeImage();

	// Definiendo la textura a utilizar
	Texture textureSponge("../Textures/sponge2.jpg");
	// Carga el mapa de bits (FIBITMAP es el tipo de dato de la libreria)
...
void destroy() {
	...
	glDeleteTextures(1, &textureWater2ID);
...
void applicationLoop() {
	...
	while (psi) {
		...
		shaderMulLighting.setMatrix4("view", 1, false,
				glm::value_ptr(view));
		// Settea la matriz de vista y projection al shader con multi luces y multi text
		shaderMulLightingMT.setMatrix4("projection", 1, false,
					glm::value_ptr(projection));
		shaderMulLightingMT.setMatrix4("view", 1, false,
				glm::value_ptr(view));
		// Settea la matriz de vista y projection al shader con multi luces y multi text
		shaderMulLighting_T03.setMatrix4("projection", 1, false,
					glm::value_ptr(projection));
		shaderMulLighting_T03.setMatrix4("view", 1, false,
				glm::value_ptr(view));
		...
		shaderMulLightingMT.setVectorFloat3(
			"viewPos", glm::value_ptr(camera->getPosition()));
		shaderMulLightingMT.setVectorFloat3(
			"directionalLight.light.ambient", glm::value_ptr(glm::vec3(0.6, 0.6, 0.6)));
		shaderMulLightingMT.setVectorFloat3(
		...
		shaderMulLighting_T03.setVectorFloat3(
			"viewPos", glm::value_ptr(camera->getPosition()));
		shaderMulLighting_T03.setVectorFloat3(
			"directionalLight.light.ambient", glm::value_ptr(glm::vec3(0.6, 0.6, 0.6)));
		shaderMulLighting_T03.setVectorFloat3(
		...
		shaderMulLighting_T03.setInt("spotLightCount", 0);
		...
		*********************************************/
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, textureWallID);
		shaderMulLightingMT.setInt("texture1", 0); // GL_TEXTURE0
		glActiveTexture(GL_TEXTURE3);
		glBindTexture(GL_TEXTURE_2D, textureWindowID);
		shaderMulLightingMT.setInt("texture2", 3); // GL_TEXTURE3
		...
		shaderMulLightingMT.setInt("texture2", 0); // GL_TEXTURE0
		glActiveTexture(GL_TEXTURE3);
		glBindTexture(GL_TEXTURE_2D, textureWindowID);
		shaderMulLightingMT.setInt("texture1", 3); // GL_TEXTURE3
		...
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, textureWater2ID);
		shaderMulLighting_T03.setInt("texture1", 0); // GL_TEXTURE0
		glActiveTexture(GL_TEXTURE3);
		glBindTexture(GL_TEXTURE_2D, textureSpongeID);
		shaderMulLighting_T03.setInt("texture2", 3); // GL_TEXTURE3

		boxEsponja.setScale(glm::vec3(1.0, 1.0, 1.0));
		boxEsponja.setPosition(glm::vec3(3.0f, 2.0f, -20.0f));
		boxEsponja.render();
